...
public class VehicleMovement : MonoBehaviour
{
    ...
    IShipInput input;                       //A reference to the player's input					
    ...
    void CalculatePropulsion()
    {
        ...
        float propulsion = driveForce * input.Thruster;
        float dragAmount = drag * Mathf.Clamp(speed, 0f, terminalVelocity);
        ...
        if (dragAmount > Mathf.Abs(propulsion))
        {
            propulsion = 0;
        }
        else
